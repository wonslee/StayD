...
public class FXUtils {
    ...
    /**
     * 씬을 변경하는 메서드. 지정된 FXML 파일을 로드하고, 새로운 Scene을 설정하여 화면 전환을 처리합니다.
     *
    ...
    /**
     * 알림창을 공용으로 띄우는 메서드. 주어진 타입과 메시지를 기반으로 알림창을 생성하고, 해당 알림을 화면에 띄웁니다.
     *
    ...
    public static void navigateToPage(String fxmlPath, ActionEvent event) {
        try {
            ...
            e.printStackTrace();
            showAlert(((Node) event.getSource()).getScene().getWindow(), Alert.AlertType.ERROR, "화면 전환 오류",
                    "오류가 발생했습니다.");
        }
    }

    /**
     * MouseEvent를 처리할 수 있도록 화면 전환을 수행하는 메서드. 마우스 클릭 이벤트 시 해당 페이지로 이동합니다.
     *
     * @param fxmlPath 이동할 FXML 파일 경로
