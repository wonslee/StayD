...
CREATE OR REPLACE PROCEDURE create_study_cafe(
    ...
    p_operating_days IN VARCHAR2,  -- "MON,TUE,WED,THU,FRI" 형태 (영어)
...
BEGIN
    ...
    BEGIN
        DBMS_OUTPUT.PUT_LINE('PL/SQL 카페 생성 시작 (영어 요일 저장)...');
        DBMS_OUTPUT.PUT_LINE('   운영일 문자열: ' || p_operating_days);
        ...
        -- 2. 운영시간 파싱 및 생성 (영어 요일로 저장)
        -- "MON,TUE,WED,THU,FRI" 문자열을 파싱하여 각각 INSERT
        WHILE v_start_pos <= LENGTH(p_operating_days) LOOP
                ...
                -- 요일이 비어있지 않고 유효한 영어 요일인지 확인
                IF LENGTH(v_day) > 0 AND v_day IN ('MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN') THEN
                ...
                ELSE
                    DBMS_OUTPUT.PUT_LINE('Warning: Invalid day format: ' || v_day);
                END IF;
            END LOOP;
        ...
        DBMS_OUTPUT.PUT_LINE('   운영일 수: ' || v_days_processed || '개 (영어 저장)');
